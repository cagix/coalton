...
Division defaults to `Double-Float` operations when given `Integer` values. [see Instance Defaulting](#instance-defaulting).
...
But an `Integer` result from division is not possible.
